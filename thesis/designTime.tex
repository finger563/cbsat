...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{figs/network_diagram_cyphy.png}
  \caption{Diagram of the testbed network and setup.  The CPS nodes are
    connected to each other, but their network communication is routed
    through the traffic shaping and delay node.}
...
switch. We implemented the network link profile using a traffic
shaping node through which all network traffic flowed.  On this node
we ran dummynet\cite{Dummynet2010}, which can be configured to control
bandwidth, latency, and packet loss on a per-link basis.  For these
experiments, we configured the traffic shaping node to control the
data rate of the application data on the network interface according
to the system provided network profile.  This testbed setup is shown
in Figure~\ref{fig:network_diagram}.
