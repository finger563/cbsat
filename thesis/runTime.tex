...
\begin{figure}[ht!]
  ...
  \caption{Demonstration of the accuracy with which our traffic
    generators follow the specified profile.  The sloped regions
...
these profiles describe the $\mu$ and $MDR$ of the
...
sending data at a rate $DR_i > \mu_i$.  The assumption implicit
in this comparison is that the sender, to be able to make this
comparison, has full knowledge of $\mu_i$, since $DR_i$ is
...
the sender's $\mu$.  If the comparison is \emph{true}, it uses the
